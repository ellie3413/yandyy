...
//코드 합치는 부분
  ...
  const [uploadedImageUrl, setUploadedImageUrl] = useState(null); // cloud URL
  const handleOpenPopup = async () => {
    ...
    // 2) 정사각으로 크롭
    const side  = Math.min(original.width-700, original.height-700);
    ...
    form.append('upload_preset', 'yandyy');
    const res = await fetch(
      'https://api.cloudinary.com/v1_1/dhbymv9gw/image/upload',
    ...
    const handleShare = async (message) => {
      ...
      window.Kakao.Share.sendCustom({
          templateId: 121899,
          templateArgs: {
            THU: uploadedImageUrl,////여기가 수정해야함, 이미지 주소 넣어야함
            message: message,
          },
        });
    };
